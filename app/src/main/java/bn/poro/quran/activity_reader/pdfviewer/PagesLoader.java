...
import static bn.poro.quran.activity_reader.pdfviewer.util.Constants.CACHE_SIZE;
import static bn.poro.quran.activity_reader.pdfviewer.util.Constants.PRELOAD_OFFSET;
...
import bn.poro.quran.activity_reader.pdfviewer.util.Util;
...
class PagesLoader {
    ...
    private static class RenderRange {
        ...
        final GridSize leftTop;
        final GridSize rightBottom;

        RenderRange() {
            this.gridSize = new GridSize();
            this.leftTop = new GridSize();
            this.rightBottom = new GridSize();
        }
    }
    ...
    private void getPageColsRows(GridSize grid, int pageIndex) {
        ...
        grid.row = MathUtils.ceil(1f / partHeight);
        grid.col = MathUtils.ceil(1f / partWidth);
    }

    private void calculatePartSize(GridSize grid) {
        pageRelativePartWidth = 1f / (float) grid.col;
        pageRelativePartHeight = 1f / (float) grid.row;
    ...
    private List<RenderRange> getRenderRangeList(float firstXOffset, float firstYOffset, float lastXOffset, float lastYOffset) {
        ...
        for (int page = firstPage; page <= lastPage; page++) {
            ...
            float rowHeight = scaledPageSize.getHeight() / range.gridSize.row;
            float colWidth = scaledPageSize.getWidth() / range.gridSize.col;
    ...
    private void loadThumbnail(int page) {
        ...
        if (!pdfView.cacheManager.containsThumbnail(page, thumbnailRect)) {
            pdfView.renderingHandler.addRenderingTask(page,
                    ...
                    true, 0, false, pdfView.isAnnotationRendering());
        }
    }
