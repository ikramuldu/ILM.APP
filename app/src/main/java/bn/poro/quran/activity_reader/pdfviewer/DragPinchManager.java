...
import bn.poro.quran.activity_reader.pdfviewer.model.LinkTapEvent;
...
import static bn.poro.quran.activity_reader.pdfviewer.util.Constants.MAXIMUM_ZOOM;
import static bn.poro.quran.activity_reader.pdfviewer.util.Constants.MINIMUM_ZOOM;
...
class DragPinchManager implements GestureDetector.OnGestureListener,
    ...
    public boolean onSingleTapConfirmed(MotionEvent e) {
        ...
        if (!linkTapped) {
            ...
            if (ps != null && pdfView.documentNotFitsView()) {
                if (!ps.shown()) {
                    ps.show();
                } else {
                    ps.hide();
    ...
    public boolean onDoubleTap(@NonNull MotionEvent e) {
        ...
        if (pdfView.getZoom() < 2) {
            pdfView.zoomWithAnimation(e.getX(), e.getY(), 2);
        } else if (pdfView.getZoom() < 4) {
            pdfView.zoomWithAnimation(e.getX(), e.getY(), 4);
        } else {
    ...
    public boolean onFling(MotionEvent e1, @NonNull MotionEvent e2, float velocityX, float velocityY) {
        ...
        //if (pdfView.isLocked()) velocityX = 0;
        if (pdfView.isPageFlingEnabled()) {
            if (pdfView.pageFillsScreen()&&!pdfView.isLocked()) {
    ...
    public boolean onScale(@NonNull ScaleGestureDetector detector) {
        if (pdfView.isLocked()) return true;
