...
public class CheckExpandabilityTask extends Thread {
    ...
    public void run() {
        SQLiteDatabase database = SQLiteDatabase.openDatabase(Utils.dataPath
                + Consts.HADIS_SUB_PATH + adapter.activity.bookID + ".db", null, SQLiteDatabase.OPEN_READONLY | SQLiteDatabase.NO_LOCALIZED_COLLATORS);
        DrawerAdapter.DrawerItem currentItem = adapter.drawerItems.get(start);

        Handler handler = new Handler(Looper.getMainLooper());
        while (start < end) {
            start++;
            DrawerAdapter.DrawerItem nextItem;
            int nextId;
            ...
            Cursor cursor = database.rawQuery("select type from content where id between ? and ? and type>? limit 1", new String[]{
                    String.valueOf(currentItem.id + 1),
